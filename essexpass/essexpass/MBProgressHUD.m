...
static NSString *kLabelFontStyle = @"Open Sans";
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	MBProgressHUD *hud = [[self alloc] initWithView:view Tabbar:nil];
	[view addSubview:hud];
	[hud show:animated];
	return MB_AUTORELEASE(hud);
}

+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view Tabbar:(UITabBar *)tab animated:(BOOL)animated {
    MBProgressHUD *hud = [[self alloc] initWithView:view Tabbar:tab];
    [view addSubview:hud];
    [hud show:animated];
    return MB_AUTORELEASE(hud);
}
...
//        self.labelFont = [UIFont fontWithName:kLabelFontStyle size:kLabelFontSize];
...
- (id)initWithFrame:(CGRect)frame Tabbar:(UITabBar *)tab {
    self = [super initWithFrame:frame ];
    if (self) {
        // Set default values for properties
        self.animationType = MBProgressHUDAnimationZoom;
...
- (id)initWithView:(UIView *)view Tabbar:(UITabBar *)tab{
	NSAssert(view, @"View must not be nil.");
    return [self initWithFrame:view.bounds Tabbar:tab];
}

- (id)initWithWindow:(UIWindow *)window {
	return [self initWithView:window Tabbar:nil];
}
